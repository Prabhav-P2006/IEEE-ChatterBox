...
struct MeshNode {
    ...
    explicit MeshNode(const char* name)
        ...
        printf("[CORE] Node '%s' created at %p\n", name, (void*)this); fflush(stdout);
        // Wire engine callbacks to our C callbacks
        engine.onSendNeeded([this](const std::vector<uint8_t>& data) {
            printf("[CORE] Node '%p' triggered onSendNeeded (%d bytes)\n", (void*)this, (int)data.size()); fflush(stdout);
            if (send_cb) send_cb(data.data(), (int)data.size(), send_ud);
            else printf("[CORE] WARNING: No send_cb registered for node %p\n", (void*)this); fflush(stdout);
        });
        engine.onMessageReceived([this](const std::string& sender, const std::string& text) {
            printf("[CORE] Node '%p' received message from %s: %s\n", (void*)this, sender.c_str(), text.c_str()); fflush(stdout);
            if (msg_cb) msg_cb(sender.c_str(), text.c_str(), msg_ud);
        });
        engine.onHandshakeComplete([this](const std::string& peer) {
            printf("[CORE] Node '%p' handshake COMPLETE with %s\n", (void*)this, peer.c_str()); fflush(stdout);
